...
/*
 * One shape for every quick filter, so the bar reads as a set of peers and
 * the only thing that varies is colour, which now means something: amber for
 * the two views that exist because work is waiting on a human.
 */
const CHIP_BASE =
  "flex min-h-[42px] flex-shrink-0 items-center gap-1.5 rounded-lg border px-3 text-[13px] font-semibold transition md:min-h-[34px]";
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="border-b border-slate-200 bg-white shadow-sm">
        <div className="px-3 md:px-8 py-3 md:py-4">
          ...
          {active === "bookings" && (
            <div className="sticky top-0 z-30 -mx-3 mt-3 flex items-center gap-1.5 overflow-x-auto border-y border-slate-100 bg-white px-3 py-2 md:static md:mx-0 md:flex-wrap md:gap-2 md:border-0 md:bg-transparent md:p-0">
              <div className="flex flex-shrink-0 rounded-lg border border-slate-200 bg-slate-100 p-0.5">
                {(["me", "all"] as const).map((scope) => (
                  <button
                    key={scope}
                    type="button"
                    onClick={() => setAssignmentScope(scope)}
                    className={`rounded-md px-2.5 py-2 text-[13px] font-semibold md:py-1.5 ${
              ...
              <button
                ...
                className={`md:hidden flex min-h-[42px] w-[42px] flex-shrink-0 items-center justify-center rounded-lg border ${
                  showCalendarMobile
              ...
              {/*
                * Ordered by what needs a human, not alphabetically.
                *
                * Pending and Unassigned used to sit fifth and sixth in a strip
                * that scrolls off a 390px phone, so the two views that exist to
                * catch work falling through the cracks were the two you had to
                * go looking for. They now follow Today directly.
                */}
              <button
                type="button"
              ...
              {unassignedCount > 0 && (
                ...
                >
                  <svg className="h-3.5 w-3.5 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" aria-hidden="true">
                    <path d="M12 9v4" />
                    <path d="M12 17h.01" />
              ...
              >
                Confirmed
                <span className="tabular-nums opacity-70">{confirmedCount}</span>
              </button>
              ...
              >
                Completed
                <span className="tabular-nums opacity-70">{completedCount}</span>
              </button>

              <div className="flex flex-shrink-0 items-center gap-1.5 md:ml-auto">
                <button
                  type="button"
                  onClick={fetchAll}
                  className="flex min-h-[42px] w-[42px] flex-shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 md:min-h-[34px] md:w-[34px]"
                  aria-label="Refresh"
                >
