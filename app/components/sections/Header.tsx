    ...
    <header className="w-full bg-transparent py-[20px] max-w-[1240px] mx-auto relative z-50">
      <div className="container mx-auto px-[20px] flex items-center justify-between max-w-[1240px]">
        ...
        <nav className="hidden lg:flex items-center gap-8">
          <Link
            href="#how-it-works"
            className="text-[#313234] hover:text-[#306EEC] transition-colors text-base font-normal relative group pb-2"
          >
            How it works
        ...
        <div className="hidden lg:flex items-center gap-4">
          {isAuthenticated ? (
            <div className="relative" ref={profileMenuRef}>
              ...
              >
                <span className="text-[#313234] text-base">{firstName}</span>
            ...
            <>
              <Link
                href="/signin"
                className="px-6 py-3 text-[#313234] hover:text-[#306EEC] border border-[#313234] rounded-[14px] text-base font-normal transition-colors hover:bg-black/5"
              >
                Sign in
              </Link>
              <Link
        ...
        >
          <span className={`w-6 h-0.5 bg-[#313234] transition-all duration-300 ${isMenuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
          <span className={`w-6 h-0.5 bg-[#313234] transition-all duration-300 ${isMenuOpen ? 'opacity-0' : ''}`}></span>
          <span className={`w-6 h-0.5 bg-[#313234] transition-all duration-300 ${isMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
        </button>
      </div>
      ...
      >
        <nav className="flex flex-col items-center justify-center h-full gap-8 px-8">
          <Link
            ...
            className="text-[#313234] hover:text-[#306EEC] text-2xl font-normal transition-colors"
          >
            How it works
          </Link>
          <Link
            ...
            {isAuthenticated ? (
              <>
                <div className="flex items-center justify-center gap-3 mb-4">
                  ...
                  <span className="text-[#313234] text-xl font-medium">{firstName}</span>
                </div>

                <Link
                  href="/account"
                  onClick={() => setIsMenuOpen(false)}
                  className="w-full text-center px-6 py-3 text-[#313234] hover:text-[#306EEC] border border-[#313234] rounded-[14px] text-base font-normal transition-colors hover:bg-black/5"
                >
                  My Account
                </Link>
