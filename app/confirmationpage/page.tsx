...
export default function ConfirmationPage() {
  ...
  useEffect(() => {
    ...
    // ✅ If user opens this page directly (no session_id), do nothing (don’t fire fake $0 purchase)
    if (!sessionId) return;

    // Fire purchase event only once per session (prevents duplicates)
    ...
    (async () => {
      try {
        ...
        // If backend lookup fails, don’t fire a fake purchase
        // (Better to miss one event than contaminate revenue data)
      }
    })();
  }, [sessionId]);
